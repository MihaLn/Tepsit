...
public class Jparser 
{
	...
	public Jparser(String file1, String file2)//costruttore a cui viene passato il nome dei due file 
	{
		this.filename = new String[2];
		this.filename[0] = file1;
		this.filename[1] = file2;
	}
	...
	private void mergeFiles(Element root1, Element root2, Document doc1)//aggiunge in root1 i figli di root 2 (profondità: primo figlio della radice)
	{	
		NodeList films = root2.getElementsByTagName("Film");
	...
	public void parseAndPrint(boolean save) throws ParserConfigurationException, SAXException, IOException //INIZIA DA QUI
	{
		...
		Element[] roots = new Element[2];
		String salvataggio = "<Canale>\n";
		
		for(int i=0; i<2; i++)//genera i due alberi
		{
			docs[i] = parseFile(this.filename[i]);
			roots[i] = docs[i].getDocumentElement();
		}
		
		addChild("Pinocchio","Animazione","20:30",docs[1],roots[1]);
		mergeFiles(roots[0],roots[1], docs[0]);
